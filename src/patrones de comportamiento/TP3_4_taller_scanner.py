...
class State:
	...
	def scan(self):
		...
		self.pos += 1
		if self.pos == len(self.stations) + len(self.memorized_freqs):
			self.pos = 0

		if self.pos < len(self.stations):
			print("Sintonizando... Estación {} {}".format(self.stations[self.pos], self.name))
		else:
			print("Sintonizando... Frecuencia memorizada {} {}".format(self.memorized_freqs[self.pos - len(self.stations)], self.name))
