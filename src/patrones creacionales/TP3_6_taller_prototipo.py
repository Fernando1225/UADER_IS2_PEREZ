...
for i in range(4):
    ...
    dt = datetime.now()
    print(f'Finaliza la creación de cuadruples mediante clone {i} at: ', dt)
    for j in range(2):
        shopkeeper_clone2 = shopkeeper_clone.clone()
        warrior_clone2 = warrior_clone.clone()
        mage_clone2 = mage_clone.clone()
        time.sleep(0.05)
        dt = datetime.now()
        print(f'Finaliza la creación de dobles mediante clone {i}-{j} at: ', dt)
        for k in range(3):
            shopkeeper_clone3 = shopkeeper_clone2.clone()
            warrior_clone3 = warrior_clone2.clone()
            mage_clone3 = mage_clone2.clone()
            time.sleep(0.05)
            dt = datetime.now()
            print(f'Finaliza la creación de tripletes mediante clone {i}-{j}-{k} at: ', dt)
    time.sleep(2)
dt = datetime.now()
